...
const FAIL_COPY: Record<PeekFail['reason'], { title: string; body: string }> = {
  not_found: {
    ...
    body: 'Este link no coincide con ninguna invitación válida. Revisá la URL o pedile a quien te invitó que te mande una nueva.',
  },
  used: {
    title: 'Invitación ya usada',
    body: 'Esta invitación ya fue aceptada. Si no fuiste vos, pedile al admin de la cuenta que mande un link nuevo.',
  },
  expired: {
    title: 'Invitación vencida',
    body: 'Esta invitación venció. Pedile al admin de la cuenta que mande una nueva — tardan solo unos segundos en generarse.',
  },
  server_error: {
    title: 'Algo salió mal',
    body: 'No pudimos verificar esta invitación ahora. Probá recargar la página en un momento.',
  },
};
...
export default function JoinPage() {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (res.status === 409) {
          setConflictMessage(
            payload.error ||
              'Ya estás en otra cuenta. Iniciá sesión con un email distinto para unirte a esta.',
          );
        } else {
  ...
  const handleSignOutAndRetry = useCallback(async () => {
    ...
    try {
      ...
      toast.error('No se pudo cerrar sesión. Probá recargar la página.');
      setSigningOut(false);
    }
  }, []);
        ...
        <Card className="w-full max-w-md border-border bg-card">
          ...
          <CardContent className="flex flex-col gap-3">
            ...
            <p className="text-center text-xs text-muted-foreground">
              ...
              cuenta personal vacía del registro se va a limpiar.
            </p>
          </CardContent>
        </Card>
          ...
          <DialogContent className="bg-popover border-border sm:max-w-md">
            ...
            <div className="space-y-2 py-2 text-xs text-muted-foreground">
              <p>
                ...
                cerrá sesión y registrate de nuevo con un email distinto.
                El link de invitación sigue siendo válido mientras no
                haya vencido.
              </p>
            </div>
