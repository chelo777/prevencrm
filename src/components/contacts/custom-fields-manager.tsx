      ...
      <DialogContent className="border-border bg-popover text-popover-foreground sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription className="text-muted-foreground">
            Definí campos extra para los contactos (ej. código postal, origen
            del lead). Aparecen en todos los contactos y en la acción de
...
export function CustomFieldsPanel() {
  ...
  async function handleDelete(field: CustomField) {
    if (
      !window.confirm(
        `¿Borrar "${field.field_name}"? También se borra su valor guardado en todos los contactos. No se puede deshacer.`
      )
    ) {
      return;
    }
    ...
    setBusyId(null);
    if (error) {
      toast.error('No se pudo borrar el campo.');
      return;
    }
    toast.success(`Borrado "${field.field_name}".`);
    await fetchFields();
  }
