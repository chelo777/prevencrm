...
export function ImportModal({
  ...
  async function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (rows.length === 0) {
      toast.error(
        'No se encontraron filas válidas. El CSV debe tener una columna "phone".'
      );
      setParsedRows([]);
      setHasTagsColumn(false);
      setHasCompanyColumn(false);
  ...
  async function handleImport() {
    ...
    try {
      ...
      try {
        ...
        toast.warning('Se importaron los contactos, pero fallaron algunas etiquetas.');
      }
      ...
      if (tagsAssigned > 0) {
        toast.success(
          `${tagsAssigned} etiqueta${tagsAssigned !== 1 ? 's' : ''} asignada${tagsAssigned !== 1 ? 's' : ''}`
        );
      }
      if (skippedNames.length > 0) {
        ...
        toast.info(
          `Etiquetas desconocidas omitidas (creálas primero en Configuración): ${sample}${more}`
        );
      }
      if (skipped > 0) {
        toast.info(`${skipped} duplicado${skipped !== 1 ? 's' : ''} omitido${skipped !== 1 ? 's' : ''}`);
      }
      if (failed > 0) {
        toast.error(
          failed === 1
            ? '1 contacto no se pudo importar'
            : `${failed} contactos no se pudieron importar`
        );
      }
    } catch (err: unknown) {
        ...
        <div className="shrink-0 space-y-4 border-b border-border/80 px-6 pt-6 pb-5">
          <DialogHeader className="gap-1.5">
            ...
            <DialogDescription className="leading-relaxed text-muted-foreground">
              Subí un CSV con la columna obligatoria{' '}
              ...
              (separadas por coma; entrecomillá las celdas con varias etiquetas).
            </DialogDescription>
          </DialogHeader>
            ...
            {file ? (
              ...
              <>
                ...
                <p className="text-sm text-muted-foreground">
                  Tocá para elegir un archivo CSV
                </p>
            ...
            <div className="space-y-3">
              ...
              {parsedRows.length > PREVIEW_LIMIT && (
                <p className="text-center text-[11px] text-muted-foreground">
                  ...
                  {parsedRows.length - PREVIEW_LIMIT !== 1 ? 's' : ''} más sin mostrar
                </p>
              )}
            </div>
          )}
              ...
              <div className="mt-3 flex flex-wrap gap-3">
                ...
                {result.tagsAssigned > 0 && (
                  <div className="flex items-center gap-1.5 text-sm text-cyan-400">
                    ...
                    {result.tagsAssigned !== 1 ? 's' : ''} asignada{result.tagsAssigned !== 1 ? 's' : ''}
                  </div>
                )}
                {result.skipped > 0 && (
                ...
                {result.failed > 0 && (
                  <div className="flex items-center gap-1.5 text-sm text-red-400">
                    ...
                    {result.failed} {result.failed !== 1 ? 'fallaron' : 'falló'}
                  </div>
                )}
              </div>
            </div>
