    ...
    <section className="rounded-xl border border-border bg-card">
      <header className="flex items-center justify-between gap-3 border-b border-border px-5 py-4">
        <div>
          ...
          <p className="mt-0.5 text-xs text-muted-foreground">
            Minutos hasta responder el primer mensaje sin contestar, por día
            de la semana
          </p>
        </div>
      ...
      <div className="p-5">
        {loading || !data ? (
          ...
          <EmptyState
            icon={Clock}
            title="Todavía no hay respuestas registradas"
            hint="Este gráfico se llena a medida que respondés mensajes de clientes."
          />
        ) : (
          <BarChart
            data={chartData}
