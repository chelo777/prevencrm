...
export function DealForm({
  ...
  async function handleSave() {
    ...
    if (deal) {
      const { error } = await supabase
        ...
        .eq("id", deal.id);
      if (error) {
        toast.error("No se pudo guardar el deal");
        setSaving(false);
        return;
      }
    } else {
  ...
  async function handleStatusChange(status: DealStatus) {
    ...
    if (error) {
      toast.error("No se pudo actualizar el estado del deal");
      return;
    }
    toast.success(
      status === "won" ? "Marcado como ganado" : status === "lost" ? "Marcado como perdido" : "Deal reabierto",
    );
    onOpenChange(false);
    onSaved();
  }
  ...
  async function handleDelete() {
    ...
    setDeleting(false);
    if (error) {
      toast.error("No se pudo borrar el deal");
      return;
    }
    toast.success("Deal borrado");
    setConfirmDelete(false);
    onOpenChange(false);
    onSaved();
  }
      ...
      >
        <div className="flex h-full flex-col">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            ...
            <div className="grid gap-2">
              <Label className="text-muted-foreground">Fecha estimada de cierre</Label>
              <Input
                type="date"
            ...
            {deal &&
              (confirmDelete ? (
                <div className="mt-3 flex items-center justify-between gap-2 rounded-md border border-red-500/30 bg-red-500/10 px-3 py-2 text-xs">
                  <span className="text-red-300">¿Borrar este deal?</span>
