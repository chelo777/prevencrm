...
export function PipelineSettings({
  ...
  async function handleRemoveStage(stageId: string) {
    ...
    const { count } = await supabase
      ...
      .eq("stage_id", stageId);
    if (count && count > 0) {
      toast.error("Primero mové o borrá los deals de esta etapa");
      return;
    }
    const { error } = await supabase
      ...
      .eq("id", stageId);
    if (error) {
      toast.error("No se pudo borrar la etapa");
      return;
    }
  ...
  async function handleDeletePipeline() {
    ...
    setDeleting(false);
    if (error) {
      toast.error("No se pudo borrar el pipeline");
      return;
    }
    onOpenChange(false);
    onPipelinesChanged();
    toast.success("Pipeline borrado");
  }
        ...
        {showDeleteConfirm ? (
          <div className="py-4">
            <div className="flex items-center gap-3 rounded-lg border border-red-500/30 bg-red-500/10 p-4">
              ...
              <div>
                ...
                <p className="mt-1 text-xs text-muted-foreground">
                  Se borra el pipeline con todos sus deals. No se puede
                  deshacer.
                </p>
              </div>
            </div>
            <div className="mt-4 flex justify-end gap-2">
              ...
              >
                {deleting ? "Borrando…" : "Borrar pipeline"}
              </Button>
            </div>
          </div>
        ) : (
