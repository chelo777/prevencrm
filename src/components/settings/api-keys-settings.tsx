...
export function ApiKeysSettings() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      if (!res.ok) {
        ...
        toast.error(payload.error || 'No se pudieron cargar las claves de API');
        return;
      }
  ...
  async function handleRevoke(key: ApiKey) {
    ...
    try {
      ...
      if (!res.ok) {
        const payload = await res.json().catch(() => ({}));
        toast.error(payload.error || 'No se pudo revocar la clave');
        return;
      }
      ...
      <SettingsPanelHead
        ...
        description={
          <>
            ...
            <code className="text-xs">/api/v1</code>) para que puedas construir
            tus propias automatizaciones. Enviálas como{' '}
          ...
          <CardContent className="flex flex-col items-center justify-center py-10 text-center">
            ...
            {canEditSettings ? (
              <p className="text-muted-foreground mt-1 text-xs">
                Tocá <span className="text-foreground">Nueva clave de API</span> para
                crear una.
              </p>
            ) : (
              <p className="text-muted-foreground mt-1 text-xs">
                Pedile a un admin que cree una.
              </p>
            )}
          </CardContent>
        </Card>
                  ...
                  >
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-muted-foreground mt-1.5 text-xs">
                        ...
                        {' · '}
                        {k.last_used_at
                          ? `usada por última vez el ${fmtDate(k.last_used_at)}`
                          : 'nunca usada'}
                        {k.expires_at && status !== 'expired'
                          ? ` · vence el ${fmtDate(k.expires_at)}`
                          : ''}
                      </p>
                    </div>
...
function CreateKeyDialog({
  ...
  async function handleCreate() {
    ...
    if (!trimmed) {
      toast.error('Ponele un nombre a la clave');
      return;
    }
    setSubmitting(true);
    try {
      ...
      const payload = await res.json().catch(() => ({}));
      if (!res.ok) {
        toast.error(payload.error || 'No se pudo crear la clave');
        return;
      }
  ...
  async function copyKey() {
    ...
    try {
      ...
      toast.error('Falló la copia — seleccioná y copiá a mano');
    }
  }
        ...
        {createdKey ? (
          <>
            <DialogHeader>
              <DialogTitle className="text-popover-foreground">
                Copiá tu clave de API
              </DialogTitle>
              <DialogDescription className="text-muted-foreground">
                Esta es la única vez que se muestra la clave completa. Guardala
                en un lugar seguro — si la perdés, revocala y creá una nueva.
              </DialogDescription>
            </DialogHeader>

            <div className="space-y-1.5">
              <Label className="text-muted-foreground">Clave de API</Label>
          ...
          <>
            <DialogHeader>
              ...
              <DialogDescription className="text-muted-foreground">
                Nombrala según la integración que la va a usar, y otorgá solo
