...
const EDITABLE_ROLES: { value: AccountRole; label: string; hint: string }[] = [
  { value: 'admin', label: 'Admin', hint: 'Gestiona miembros + todo' },
  { value: 'agent', label: 'Agente', hint: 'Usa funciones; sin configuración' },
  { value: 'viewer', label: 'Visor', hint: 'Solo lectura en toda la app' },
];
...
export function MembersTab() {
    ...
    try {
      ...
      if (ires) {
        if (!ires.ok) {
          ...
          toast.error(payload.error || 'No se pudieron cargar las invitaciones');
          return;
        }
  ...
  async function handleRoleChange(member: Member, nextRole: AccountRole) {
    ...
    try {
      ...
      toast.success(`${member.full_name || 'Miembro'} ahora es ${roleLabel}`);
    } catch (err) {
  ...
  async function handleRemove() {
    ...
    try {
      ...
      if (!res.ok) {
        const payload = await res.json().catch(() => ({}));
        toast.error(payload.error || 'No se pudo quitar al miembro');
        return;
      }
      toast.success(`${removingMember.full_name || 'Miembro'} eliminado`);
      setMembers((prev) =>
  ...
  return (
    <section className="animate-in fade-in-50 space-y-6 duration-200">
      <SettingsPanelHead
        ...
        description="Quiénes tienen acceso a esta cuenta. Los roles controlan qué puede hacer cada compañero."
        action={
          ...
          return (
            <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-muted-foreground">
              ...
              <span className="inline-flex items-center gap-1.5">
                ...
                {counts.offline} desconectado{counts.offline === 1 ? '' : 's'}
              </span>
              ...
              return (
                ...
                >
                  ...
                  <div className="hidden sm:block text-right text-xs text-muted-foreground">
                    Se unió el {fmtDate(member.joined_at)}
                  </div>
        ...
        <div>
          ...
          {invitations.length > 0 ? (
            <p className="mb-3 text-xs text-muted-foreground">
              Por seguridad, la URL de invitación en texto plano se muestra
              una sola vez al crearla — para volver a compartirla, revocá
              la invitación de abajo y creá una nueva.
            </p>
          ) : null}
            ...
            <Card>
              <CardContent className="flex flex-col items-center justify-center py-8 text-center">
                ...
                <p className="mt-1 text-xs text-muted-foreground">
                  Tocá <span className="text-muted-foreground">Invitar miembro</span>{' '}
                  arriba para generar un link para compartir.
                </p>
              </CardContent>
            </Card>
          ) : (
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2">
                          <span className="text-sm font-medium text-foreground">
                            {inv.label || 'Invitación sin nombre'}
                          </span>
                          <span
                        ...
                        <p className="mt-0.5 text-xs text-muted-foreground">
                          Creada el {fmtDate(inv.created_at)} · {fmtExpiresIn(inv.expires_at)}
                        </p>
                      </div>
          ...
          <DialogHeader>
            ...
            <DialogDescription className="text-muted-foreground">
              ...
              <span className="font-medium text-muted-foreground">
                {removingMember?.full_name || 'este compañero'}
              </span>{' '}
              de la cuenta? Va a cerrar sesión en esta cuenta y va a tener
              una cuenta personal nueva la próxima vez que inicie sesión.
              Su acceso no se borra.
            </DialogDescription>
          </DialogHeader>
