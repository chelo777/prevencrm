...
export function ProfileForm() {
  ...
  const onPickFile = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!ALLOWED_MIME.has(file.type)) {
      toast.error('Tipo de imagen no soportado', {
        description: 'Usá PNG, JPG, WebP o GIF.',
      });
      return;
    }
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error: updateError } = await supabase
        ...
        .eq('user_id', user.id);
      if (updateError) {
        throw new Error(`Falló el guardado: ${updateError.message}`);
      }
      ...
      if (trimmedEmail.toLowerCase() !== profile.email.toLowerCase()) {
        ...
        if (emailError) {
          ...
          toast.error(`No se pudo cambiar el email: ${emailError.message}`);
          setSaving(false);
  ...
  return (
    <section className="max-w-2xl animate-in fade-in-50 duration-200">
      <SettingsPanelHead
        title="Tu perfil"
        description="Cómo aparecés en la app. Tu avatar y nombre se muestran en el header, el sidebar y donde tus compañeros te vean."
      />
          ...
          <div className="space-y-2">
            ...
            {emailChangePending && (
              <p className="flex items-start gap-2 rounded-md border border-amber-500/30 bg-amber-500/10 px-3 py-2 text-xs text-amber-700 dark:text-amber-300">
                ...
                <span>
                  Revisá la bandeja de <strong>{profile?.email}</strong> y{' '}
                  <strong>{email}</strong> — ambos deben confirmar antes de
