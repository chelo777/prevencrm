...
export async function loadActivity(db: DB, limit = 20): Promise<ActivityItem[]> {
  ...
  for (const c of (contacts.data ?? []) as Array<{ id: string; name: string | null; phone: string; created_at: string }>) {
    items.push({
      id: `contact-${c.id}`,
      kind: 'contact',
      text: `Contacto nuevo: ${c.name || c.phone}`,
      at: c.created_at,
      href: '/contacts',
    })
  }
  ...
  for (const d of (deals.data ?? []) as unknown as Array<{
    ...
    items.push({
      ...
      kind: 'deal',
      text: stage?.name
        ? `Deal "${d.title}" en ${stage.name}`
        : `Deal "${d.title}" actualizado`,
      at: d.updated_at,
      href: '/pipelines',
    })
  }
  ...
  for (const b of (broadcasts.data ?? []) as Array<{
    ...
    const label =
      b.status === 'sent'
        ...
        : `${b.status} (${b.total_recipients} destinatarios)`
    items.push({
      id: `broadcast-${b.id}`,
      kind: 'broadcast',
      text: `Difusión "${b.name}" ${label}`,
      at: b.created_at,
      href: '/broadcasts',
    })
  }
  ...
  for (const l of (autoLogs.data ?? []) as unknown as Array<{
    ...
    items.push({
      id: `auto-${l.id}`,
      kind: 'automation',
      text: `Automatización "${autoName}" ${l.status === 'failed' ? 'falló para' : 'se disparó para'} ${who}`,
      at: l.created_at,
    })
  }
